rm -rf ~/Documents/Files/Bash/MacOS-Dotfiles/bin
rm -rf ~/Documents/Files/Bash/MacOS-Dotfiles/scripts
rm -rf ~/Documents/Files/Bash/MacOS-Dotfiles/.config
rm -rf ~/Documents/Files/Bash/MacOS-Dotfiles/.profile
rm -rf ~/Documents/Files/Bash/MacOS-Dotfiles/.skhdrc
rm -rf ~/Documents/Files/Bash/MacOS-Dotfiles/.Xresources
rm -rf ~/Documents/Files/Bash/MacOS-Dotfiles/.zshrc
rm -rf ~/Documents/Files/Bash/MacOS-Dotfiles/spicetify_data
rm -rf ~/Documents/Files/Bash/MacOS-Dotfiles/.wallpaper
rm -rf ~/Documents/Files/Bash/MacOS-Dotfiles/.tmux.conf


cp-improved $HOME/bin/ ~/Documents/Files/Bash/MacOS-Dotfiles/bin 
cp-improved $HOME/scripts ~/Documents/Files/Bash/MacOS-Dotfiles/
cp-improved $HOME/.yabairc ~/Documents/Files/Bash/MacOS-Dotfiles/
cp-improved $HOME/.config/ranger/ ~/Documents/Files/Bash/MacOS-Dotfiles/.config/ranger
cp-improved $HOME/.config/nvim/ ~/Documents/Files/Bash/MacOS-Dotfiles/.config/nvim
cp-improved $HOME/.config/alacritty ~/Documents/Files/Bash/MacOS-Dotfiles/.config/
cp-improved $HOME/.profile ~/Documents/Files/Bash/MacOS-Dotfiles/
cp-improved $HOME/.skhdrc ~/Documents/Files/Bash/MacOS-Dotfiles/
cp-improved $HOME/.Xresources ~/Documents/Files/Bash/MacOS-Dotfiles/
cp-improved $HOME/.zshrc ~/Documents/Files/Bash/MacOS-Dotfiles/
cp-improved $HOME/spicetify_data/Themes ~/Documents/Files/Bash/MacOS-Dotfiles/spicetify_data/
cp-improved $HOME/spicetify_data/config.ini ~/Documents/Files/Bash/MacOS-Dotfiles/spicetify_data/
cp-improved $HOME/.wallpaper ~/Documents/Files/Bash/MacOS-Dotfiles/
cp-improved $HOME/.tmux.conf ~/Documents/Files/Bash/MacOS-Dotfiles/


